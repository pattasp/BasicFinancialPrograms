...
// for Rainbow with Control Variates

// So we need to add to our function the method of pilot simulations
// to estimate the optimal c_ij such as Var(Xc) = minimum

double MC_Call_Price_RainbowAV(double S1,double S2,double K,double r,double v1,double v2,double T,int num_sims){
    ...
    for( i=0; i<num_sims; i++){
    	...
    	// Prices of Underlying Assets with the implementation of AV:
    	
    	double S1_T_AV = S1 * exp(nu_1 + v_1 * (-epsilon1));
    	double S2_T_AV = S2 * exp(nu_1 + v_1 * (-epsilon2));
    	
    	//Setting up the payoff of the 2 assets Rainbow Price
    	double minimum1 = min(S1_T,S2_T);
    	double minimum2 = min(S1_T_AV,S2_T_AV);
    	
    	// Now for the disc_payoff : For the estimated payoff f -> Check this again
    	// E[S_i] = E[tilde(S_i)] = θ so : Z_i = (1/2)*(S_i + tilde(S_i))
    	
    	double disc_payoff = max(minimum1 - K,0.0) * exp( - r * T) * 0.5 + max(minimum2 - K,0.0) * exp( - r * T) * 0.5;
...
int main() {
    ...
    double call_price = MC_Call_Price_RainbowAV(S1,S2,K,r,v1,v2,T,num_sims);
    ...
    cout << "Number of Paths for Antithetic Variates: " << num_sims << endl;
